...
As we've been learning, working with text files often involves line-based processing of the contents of a file.  This is very much the case when you want to search within a file or many files all at once.  To zone in on exactly what you want to find inside of files, the `+grep+` command is your friend!  Go ahead and have a look at the manual page for `+grep+`.  You'll see that it will "print lines that match patterns".  When we talk about using `+grep+` on the command line, it means we are searching for text-based patterns in one or more files, so the arguments you give to the command include at least those two things--a text pattern to search for, and a set of files to search within.  Let's begin with a common example where you want to see if a word is inside of a file.  Try searching for yourself inside of a file in our star files that are organized by tropical sign.  _Example 66_ shows how to do a simple search within the `+Virgo.txt+` file for the word `+super+`.

.Using the `+grep+` command to search within a file for a single word.
[source, console, caption="Example {counter:listing-counter}: "]
----
...
Perfect!  You found one line of text in the `+Virgo.txt+` file that contains the word `+super+`.  So in this case, the pattern we're looking for is the word "super", and the file set we are searching is just the Virgo.txt file.  So easy!  We see that the star named Suhail is an orange super giant that is 545 light years away from us.  Very cool!

==== Regular expressions in commands

Now, let's continue and search for a pattern across multiple files--all of our star files--but in this case, rather than a single word our search pattern will use what is called a _regular expression_ in computing.  Recall in _Chapter 4. Folder Commands_ where we learned about path expansion.  We used special characters like `+{}+` (curly braces) and `+[]+` (square brackets) to tell the shell we want a list or a sequence to be expanded.  Regular expressions are similar--they help you define a search pattern that represents the sequence of characters you are looking for in the text.  The `+grep+` command supports modern regular expressions using the `+-E+` command option.

TIP:  In computing, regular expressions are used in most all modern programming languages to match text-based patterns of letters, numbers, and symbols.  Learning how to use regular expressions in command line commands is a skill that can be transferred over when learning to program.

As an example, we can search the _distance_ field in our star files to find stars that are greater than 500 light years from Earth by using a regular expression with the `+grep+` command.  Notice that the distance field has four numbers followed by the letters "ly", meaning light years.  An example is `+0807ly+`.  The four numbers represent the thousands, hundreds, tens, and ones digits expressed in light years.  So to search for numbers that are greater than 500, let's use number ranges in our search pattern for each digit.  The pattern `+[0-9][5-9][0-9][0-9]ly+` defines any number matching zero through nine in the thousands digit, five through nine in the hundreds digit, and zero through nine in the tens and ones digits.  Those digits must also be followed by the text characters `+ly+`. 

Go ahead and give this a try!  Let's search all of the star files using an `+*+` (star) wildcard for file expansion.  When you search multiple files, by default the `+grep+` command displays which file the lines are found in, and in this case we use the `+-h+` option to hide the file names from being shown in the results.  Have a look at _Example 67_, which shows the `+grep+` command syntax and the sorted results.
...
[%header,cols="^1m,^4m", stripes=even]
|===
|Pattern | Meaning

<|abc…     <|All upper and lower letters
<|123…     <|All digits from zero to nine
<|\d       <|Any single digit
...
<|[0-9]    <|Numbers 0 to 9
<|{m}      <|m repetitions of the preceding pattern
<|{m,n}    <|m to n repetitions of the preceding pattern
<|*        <|Zero or more repetitions of the preceding pattern
<|+        <|One or more repetitions of the preceding pattern
<|?        <|An optional character
<|\s       <|Any whitespace (spaces, tabs, newline, carriage return)
