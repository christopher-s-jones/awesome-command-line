...
The command prompt on modern systems tend to include your username, folowed by an `+@+` symbol, followed by the network hostname of your computer.  There is usually some kind of delimiter character ( a space or colon), followed by a `+~+` tilda character (which, as we discuss later, represents your home folder). Lastly, you will see either a `+$+` dollar sign character (for `+bash+` shells) or a `+%+` character (for `+zsh+` shells).  See _Example 3_ for various command line prompt examples.
...
image::chapter-01-command-parts.png[title="The labeled parts of a command, including a subcommand, option, long option, option value, and operand, which are all types of command arguments.", pdfwidth=100%]
