...
We are now familiar with opening a terminal application, which in turn spins up a shell interpreter to handle your commands behind the scenes.  Now let's familiarize ourselves with the idea of the _command prompt_, which is your go-to location for typing in commands.  Once your terminal application has opened, you are presented with an almost empty window, with a few characters written at the top. These characters are followed by the _cursor_, which is some sort of flashing--or not flashing--block character, underscore or other inviting symbol that ever so subtlely evokes "type here".

Collectively, this is all considered the command prompt, where it is dutifully waiting for you to enter a command.  See _Figure 15_ for various examples of the command prompt.

image::chapter-01-command-prompt-examples.png[title="The command line, with various examples of a command prompt, including the default zsh prompt, the default bash prompt, a root user prompt with a vertical bar cursor, a minimalist chevron prompt with an underscore cursor, and a root user prompt with no cursor.", pdfwidth=100%]

Modern systems tend to include your username, folowed by an `+@+` symbol, followed by the network name of your computer.  There is usually some kind of delimiter character ( a space or colon), followed by a `+~+` tilda character. Lastly, you will see either a `+$+` dollar sign character (for `+bash+` shells) or a `+%+` character (for `+zsh+` shells).  What character shows up here is configurable, and some people prefer having a minimalist prompt with just a `+>+` chevron symbol, with no username or other information.  The command prompt tends to be on the very first line of your terminal window.  The combination of the command prompt, and this imaginary first line of text at the top of your window, is considered the _command line_.  This is your magic portal that gives you superpowers with your computer, which we will see in the following chapters.

NOTE: On UNIX-like operating systems like macOS and Linux, an account for the administrator (also called the super-user, or root), conventionally is denoted by a `+#+` hashtag symbol in the command prompt rather than a `+$+` or `+%+` sign.
