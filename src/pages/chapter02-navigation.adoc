...
Great! You now have the tools to navigate any manual page for any command.  As you work with commands over and over, you will remember the some of short and long options that are available because you will use them regularly.  The repetition is your friend! But there are some options that you will use infrequently, and may forget what they mean and how to use them.  You may need to open the manual page, and search for the exact option you want to use, so you can re-read the details.  This is possible too!  Let's use the `+man pwd+` command output as an example again.  When you type that command, you will see there is a `+-L+` short option for the `+pwd+` command. To keep things simple in this section, we won't discuss the option itself, but will cover the concept of _symbolic links_ in a later section.  If you want to search within a manual page, first type the `+/+` slash key on your keyboard while you are viewing the manual page, which indicates to the paging mechanism that you want to search.  Your cursor will show at the very bottom-left corner of your window.  Immediately after the `+/+`, type `+-L+`, so it looks like `+/-L+`, and press the kbd:[Return] key.  Whoosh!  The manual page scrolls directly to the first instance of the `+-L+` characters.  This shortcut can save a lot of time when you generally know what you are looking for.  It can be helpful to use `+/EXAMPLES+` to jump directly to the `EXAMPLES` section, if it exists for the command.  If not, it will tell you "Pattern not found".
