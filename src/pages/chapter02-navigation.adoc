...
As you can see, unmodified `+ls+` command output works well for having a quick look at a directory's contents, but when there are dozens or hundreds of items in a directory, it can be a bit unwieldy.  To tidy up the output, we can add a `+-1+` (numeral one) option to the command, which tells `+ls+` to list the contents in a single column.  _Example 21_ shows the results of an `+ls -1+` command.
...
.The many options available to the `+ls+` command on macOS.
[source, console, caption="Example {counter:listing-counter}: "]
----
chris@ophir ~ % man ls
...
----
<1> The listing of items in _chris'_ home folder with accompanying details

Okay--that's looking packed full!  The long listing provides not only a vertical list of folder and file names that are alphabetically sorted by default, but every line also provides technical details for each item in the list.  _Figure 23_ explains each of the columns of detail, and highlights the far right column with the subdirectory and file names.
...
Give this command a try for yourself in your home directory.  When you're looking at the output, it is helpful to envision it as columns, with the 7th column being the most important (the folder and file names).  However, the 6th column--modification dates--can be very helpful when you're interested when you've last worked on a given file or folder.  Likewise, the size of the file or folder in column 5 is useful, and common in graphical interface listings too.  Because we used the `+-h+` option, file sizes in bytes are converted to Kilobytes (K), Megabytes (M), and Gigabytes (G) to make the large numbers more readable.  But what about the first four columns?  Let's discuss those.

Because UNIX-like operating systems can have multiple user accounts, every file or directory has a set of permissions and a type that are assigned to it, which are shown in column 1.  We'll return to those in a moment.  Column two is showing how many items are "linked" to the given file or directory in the given row.  For instance, the `+Pictures+` directory has 11 direct children, which are the country folders for our cat and dog photos!

NOTE: While the long listing shows an 11, in reality there are 9 subdirectories.  The other 2 links are made up the the `.` (period) which is the directory itself, and the `+..+` (double period) which is the parent directory.  These two hidden directory names are associated with the Pictures folder as well.

To the right of the item count in Column 3, we see that the _chris_ username is the owner of each file.  Likewise, there is also a group named _chris_ on the system, and all of these rows are associated with that group in Column 4.  But let's now take a look at the most condensed of all of the columns, the permissions and types in Column 1.
